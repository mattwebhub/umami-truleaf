...
export function getProductCockpitGroupCount(websiteId: string) {
  const config = getProductCockpitConfig(websiteId);
  if (!config) return 0;

  const groups = new Set<string>();
  const add = (provenance: Provenance, events: readonly string[]) =>
    groups.add(`${provenance}:${[...events].sort().join(',')}`);
  for (const metric of config.metrics) {
    if (metric.type === 'count') add(metric.provenance, metric.events);
    else {
      add(metric.provenance, metric.numerator);
      add(metric.provenance, metric.denominator);
    }
  }
  for (const funnel of config.funnels) {
    for (const step of funnel.steps) add('browser', step.events);
  }
  for (const feature of config.features) add('browser', feature.events);
...
export async function getProductContentData({
  ...
  if (!config.content) return { enabled: true as const, content: null };

  const previous = providedPrevious ?? {
    ...current,
    ...getCompareDate('prev', current.startDate, current.endDate),
  };
  const summary = await getContentSummary({
    websiteId,
    viewEvent: config.content.viewEvent,
    engagedEvent: config.content.engagedEvent,
    propertyName: config.content.idProperty,
    current,
    previous,
    limit: config.content.limit,
  });
...
export async function getProductCockpitData({
  websiteId,
  current,
  previous: providedPrevious,
  ...
  return {
    ...
    moderation: includeModeration ? { openReviews: reviews } : null,
    content: content
      ? {
